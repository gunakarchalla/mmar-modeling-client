 ...
 *
 * Pure functions, no DI — mirrors color_util.ts.
 */

/** Label height as a fraction of the viewport height. ~4% ≈ 25 px on a 640 px canvas. */
const LABEL_SCREEN_FRACTION = 0.04;
/** World height used when the camera cannot supply one (frustums start out zeroed). */
...
export function scaleLabel(sprite: THREE.Sprite, camera: THREE.Camera | undefined): void {
    ...
    // Cameras are constructed with zeroed frustums (see global_definitions) and only
    // sized once the canvas mounts, so fall back rather than collapsing the label.
...
function buildSprite(
    ...
    // Drawn after the arrow shafts and box helpers it shares a point with — which is
    // what puts the cursor pill on top, and settles ties between translucent helpers
    // for the depth-tested name tag.
